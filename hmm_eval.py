...
# MODIFIED: import for the ascii output + ansi
import ascii_output
...
def eval_HMMs(hmm_set, file_list, feature_type='fbank'):
    ...
    Args:
        hmm_set: List of all the pretrained HMMs
    ...
    target_labels = [] #Perhaps turn into numpy
    rec_labels = [] # Also turn into numpy
    for n in range(num_files):
        fn = file_list[n] # Path to the nth file in list (fn = data/test/ap/1a.wav)
        ...
        features = feat.compute_features[feature_type](signal, fs_hz) #[num_frames, num_features]
        ...
        ascii_output.update_line('{0}/{1} "{2}": lab = "{3}", rec = "{4}"'.format(n+1, num_files, fn, lab, rec))
...
def main():
    ...
    # MODIFIED: Init string encoding before any prints
    ascii_output.init_writing()
